                  ...
                  <tbody>
                    <tr v-for="(UserData, index) in UsersData" :key="index">
                      ...
                      <td>
                        ...
                        &nbsp;
                        <button href="javascript:;" class="btn btn-danger" v-on:click="DeleteUser(--index)">
                          Delete
                        </button>
                      </td>
                    </tr>
  ...
  methods: {
    DeleteUser(index) {
      if (confirm("Do you really want to delete?")) {
        if (this.UsersData[index].id) {
          this.axios
            .delete(this.api + "/" + this.UsersData[index].id)
            .then((res) => {
              alert(res.data);
            });
        }
    ...
    updateUser() {
      if (this.name.length > 0 || this.email.length > 0) {
        ...
        this.axios
          .put(this.api + "/" + this.UsersData[this.edit_index].id, data)
          .then((res) => {
